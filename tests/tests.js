module.exports = function(test, SchemaReader) {
	...
	test( "shallow read all objects and child relationships", ( t ) => {
		var reader = setup();
		var objectNameCounts = { windowObj__c: 0, doorObj__c: 0, houseObj__c: 0 };
		...
		reader.shallowReadChildRelationships((rel, object, path, reader) => {
			objectNameCounts[object.name] += 1;
			if(relationsipsVisited[rel.relationshipName])
				relationsipsVisitedTwice[rel.relationshipName] = true;
	...
	test( "deep read all objects and child relationships", ( t ) => {
		var reader = setup();
		var objectNameCounts = { windowObj__c: 0, doorObj__c: 0, houseObj__c: 0 };
		var relationshipCount = 0;
		...
		reader.deepReadChildRelationships((rel, object, path, reader) => {
			objectNameCounts[object.name] += 1;
			if(relationsipsVisited[rel.relationshipName])
				relationsipsVisitedTwice[rel.relationshipName] = true;
			relationsipsVisited[rel.relationshipName] = true;
	...
	test( "filter by field name", ( t ) => {
		var reader = setup();
		var objectNameCounts = { windowObj__c: 0, doorObj__c: 0, houseObj__c: 0 };
		var fieldCount = 0;

		reader.shallowReadFields(SchemaReader.newFieldNameFilter('id', (field, object, path, reader) => {
			objectNameCounts[object.name] += 1;
			fieldCount++;
		}));
	...
	test( "filter by object name", ( t ) => {
		var reader = setup();
		var objectNameCounts = { windowObj__c: 0, doorObj__c: 0, houseObj__c: 0 };
		var fieldCount = 0;

		reader.shallowReadFields(SchemaReader.newObjectNameFilter('windowobj__c', (field, object, path, reader) => {
			objectNameCounts[object.name] += 1;
			fieldCount++;
		}));
	...
	test( "filter by field and object name", ( t ) => {
		var reader = setup();
		var objectNameCounts = { windowObj__c: 0, doorObj__c: 0, houseObj__c: 0 };
		var fieldCount = 0;

		reader.shallowReadFields(SchemaReader.newFieldAndObjectNameFilter('id', 'windowobj__c', (field, object, path, reader) => {
			objectNameCounts[object.name] += 1;
			fieldCount++;
		}));
	...
	test( "filter fields by custom filter", ( t ) => {
		...
		var filter = (field, object, path, reader) => {
		...
		filter = (field, object, path, reader) => {
			return path.length > 3 && 
				path[0].name === 'windowObj__c';
		};
